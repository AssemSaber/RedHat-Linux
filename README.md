...
find /path -size + 100MB                     # Find everything above 100 mega bytes
...
find /path -type f -size + 100MB             # Find all files above 100 MB
```
...
journalctl -u postgresql # Find the logs of this service
```
...
## How to deal with DB has been stopped suddenly???
```
1- sudo systemctl status postgresql     # check the status to know the reason
  active (running) | inactive | Stopped | Failed
2- sudo journalctl -u postgresql OR docker compose logs postgres      # open logs for the troubleshooting to know the root issue
3 - check the server itself
 3.1 df -h   # check the disk
 3.2 free -h # check the RAM
 3.3 top     # check CPU
4- sudo systemctl start postgresql OR docker container run hash    # rerun the service 


| Setup                         | Check status                  | Logs                           | Restart                           |
| ----------------------------- | ----------------------------- | ------------------------------ | --------------------------------- |
| PostgreSQL installed on Linux | `systemctl status postgresql` | `journalctl -u postgresql`     | `systemctl restart postgresql`    |
| PostgreSQL in Docker          | `docker ps`                   | `docker logs -f <container>`   | `docker restart <container>`      |
| PostgreSQL in Docker Compose  | `docker compose ps`           | `docker compose logs postgres` | `docker compose restart postgres` |
